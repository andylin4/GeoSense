...
| **GeoGuessr-50k** *(BLOCKED)* | Kaggle `ubitquitin/geolocation-geoguessr-images-50k` | ~50k images, 124 countries | Held-out evaluation only | Actual game screenshots with UI overlay. This is the only data matching production distribution. Heavily imbalanced -- the US is ~12k of 50k (24%), and the smallest classes hold a single image. **Never train on this.** As of 2026-08-14 `kaggle.com` is blocked by the machine's Cloudflare Zero Trust policy, so no real evaluation number exists yet. Workarounds: a HuggingFace mirror, or self-captured screenshots via `scripts/label_screenshots.py`. |
